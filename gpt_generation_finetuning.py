...
def train_sentiment_model(
    ...
    filtered_dataset = dataset.filter(lambda x: x["label"] == sentiment_label).select(range(100))  # limit to 100
...
def main():
    ...
    parser.add_argument("generated_reviews_path", type=str, help="path to save the generated reviews txt file")
    parser.add_argument("models_output_dir", type=str, help="path to save the fine-tuned models")
    ...
    with open(args.generated_reviews_path, "w", encoding="utf-8") as f:
        ...
        for idx, rvw in enumerate(positive_reviews, 1):
            f.write(f"{idx}. {rvw}\n\n")
        f.write("\nreviews generated by negative model:\n")
        for idx, rvw in enumerate(negative_reviews, 1):
            f.write(f"{idx}. {rvw}\n\n")
